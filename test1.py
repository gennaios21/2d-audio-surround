# Complete updated code for 5.0 VBAP speaker simulation with GUI
...
vbap_gain = np.array([1.0] * 5)  # 5 channels
control_buttons = {}
music_slider = None

# Speaker azimuths (degrees)
speaker_angles_deg = {
    "Center": 0,
    "Right": 45,
    "Rear Right": 135,
...
def audio_callback(outdata, frames, time, status):
    ...
    # Ensure mono
    chunk = chunk[:, 0] * volume  # shape (frames,)

    # Apply VBAP gain and distribute to 5 channels
    multi_channel = np.column_stack([chunk * g for g in vbap_gain])
    outdata[:len(multi_channel)] = multi_channel
    pointer += frames
...
def calculate_vbap_gain(source_azimuth):
    source_rad = radians(source_azimuth)
    source_vec = np.array([cos(source_rad), sin(source_rad)])

    gains = []
    for angle in speaker_angles_deg.values():
        angle_rad = radians(angle)
        speaker_vec = np.array([cos(angle_rad), sin(angle_rad)])
        gain = max(0.0, np.dot(source_vec, speaker_vec))
        gains.append(gain)
...
btn_rl = tk.Button(grid_layout, text="Rear Left (215°) 🔊", width=20, height=2, font=("Arial", 14), bg="green", command=lambda: toggle_playback("Rear Left"))
